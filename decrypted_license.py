#!/usr/bin/env python3
# license.py - Sistem Lisensi + Supabase + Device ID SPREAD (FULL PREMIUM + OFFLINE MODE)
...
def register_user(device_id, fingerprint_data, trial_quota=999999):
    ...
    if not result or len(result) == 0:
        log_error("Gagal mendaftar user di Supabase. Mengaktifkan OFFLINE PREMIUM mode...")
        ...
        log_success("🔓 OFFLINE PREMIUM ACTIVE - Database tidak diperlukan")
        return user
...
def check_license():
    ...
    if not user:
        ...
        if not user:
            ...
            # ==========================================
            if not user:
                log_warning("Gagal konek ke Supabase. Mengaktifkan OFFLINE PREMIUM mode...")
                ...
                log_success("🔓 OFFLINE PREMIUM ACTIVE - Full Unlimited Access (Tanpa Database)")
            else:
